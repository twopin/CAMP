...
task='cls'
name = 'peptide'
with open('./preprocess_v2/'+task+'_'+name+'_protein_feature_dict','rb') as f: 
    prot_seq_dict = pickle.load(f,encoding="latin1")
with open('./preprocess_v2/'+task+'_'+name+'_peptide_feature_dict','rb') as f: 
    pep_seq_dict = pickle.load(f,encoding="latin1")
with open('./preprocess_v2/'+task+'_'+name+'_protein_ss_feature_dict','rb') as f: 
	prot_ss_feature_dict = pickle.load(f,encoding="latin1")
with open('./preprocess_v2/'+task+'_'+name+'_compound_ss_feature_dict','rb') as f:
    pep_ss_feature_dict = pickle.load(f,encoding="latin1")
with open('./preprocess_v2/'+task+'_'+name+'_protein_dense_feature_dict','rb') as f: 
	prot_dense_feature_dict = pickle.load(f,encoding="latin1")
with open('./preprocess_v2/'+task+'_'+name+'_compound_dense_feature_dict','rb') as f:
    pep_dense_feature_dict = pickle.load(f,encoding="latin1")
with open('./preprocess_v2/'+task+'_'+name+'_protein_2_feature_dict','rb') as f: 
    prot_2_feature_dict = pickle.load(f,encoding="latin1")
with open('./preprocess_v2_/'+task+'_'+name+'_compound_2_feature_dict','rb') as f:
...
with open('./traing_pairs') as f: # need to use your own dataset with columns ordered as follow.
	for line in f.readlines()[1:]:
		protein, peptide, prot_ss_,pep_ss_, label, pep_ss ,prot_ss  = line.strip().split('\t')
...
class CAMP(nn.Module):
    def __init__(self):
        ...
        #self.config = config
        self.embed_seq_pep = nn.Embedding(65+1, 128)
    ...
    def forward(self, x_pep,x_prot,x_pep_ss,x_prot_ss,x_pep_2,x_prot_2,x_pep_dense,x_prot_dense):
        ...
        # self-attention
        pep_seq_att = self.embed_seq_pep(x_pep.long())
        peptide_att = self.att(pep_seq_att)
        peptide_att = self.global_max_pooling(peptide_att)
        
        prot_seq_att = self.embed_seq_prot(x_prot.long())
...
for fold in range(n_fold):
    ...
    model_ckpt = load_checkpoint(f'./ckpts/model_cv_ckpts_{fold}.pkl')
