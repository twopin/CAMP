...
def align_one(ssw, qProfile, rNum, nRLen, nOpen, nExt, nFlag, nMaskLen):
    ...
    """
    res = ssw.ssw_align(
        qProfile, rNum, ct.c_int32(nRLen), nOpen, nExt, nFlag, 0, 0, nMaskLen
    )
...
def buildPath(q, r, nQryBeg, nRefBeg, lCigar):
    ...
    for x in lCigar:
        ...
        if c == "M":
            sQ += q[nQOff : nQOff + n]
            sA += "".join(
                ["|" if q[nQOff + j] == r[nROff + j] else "*" for j in xrange(n)]
            )
            sR += r[nROff : nROff + n]
            nQOff += n
            nROff += n
...
def main(args):
    ...
    if args.bSam and args.bHeader and args.bPath:
        ...
        for sRId, sRSeq, _ in read(args.target):
            print
            "@SQ\tSN:{}\tLN:{}".format(sRId, len(sRSeq))
    ...
    for sQId, sQSeq, sQQual in read(args.query):
        ...
        for sRId, sRSeq, _ in read(args.target):
            ...
            res = align_one(
                ssw, qProfile, rNum, len(sRSeq), args.nOpen, args.nExt, nFlag, nMaskLen
            )
            # align rc query
            resRc = None
            if args.bBest and not args.bProtien:
            ...
            if resRc == None or res[0] > resRc[0]:
                ...
                resPrint = resRc
                strand = 1
                sCigar, sQ, sA, sR = buildPath(
                    sQRcSeq, sRSeq, resRc[4], resRc[2], resRc[8]
                )

            # print results
            if not args.bSam:
                print
                "target_name: {}\nquery_name: {}\noptimal_alignment_score: {}\t".format(
                    sRId, sQId, resPrint[0]
                ),
                if resPrint[1] > 0:
                    print
                    "suboptimal_alignment_score: {}\t".format(resPrint[1]),
                if strand == 0:
                    print
                    "strand: +\t",
                else:
                ...
                if resPrint[2] + 1:
                    print
                    "target_begin: {}\t".format(resPrint[2] + 1),
                print
                "target_end: {}\t".format(resPrint[3] + 1),
                if resPrint[4] + 1:
                    print
                    "query_begin: {}\t".format(resPrint[4] + 1),
                print
                "query_end: {}\n".format(resPrint[5] + 1)
                if resPrint[-2] > 0:
                    n1 = 1 + resPrint[2]
                    ...
                    for i in range(0, len(sQ), 60):
                        print
                        "Target:{:>8}\t{}\t{}".format(n1, sR[i : i + 60], n2)
                        n1 = n2 + 1
                        n2 = (
                            n2
                            + min(60, len(sR) - i - 60)
                ...
                if resPrint[0] == 0:
                    ...
                    mapq = int(
                        -4.343
                        * math.log(
                            1 - abs(resPrint[0] - resPrint[1]) / float(resPrint[0])
                        )
                    )
                    mapq = int(mapq + 4.99)
                    if mapq >= 254:
                    ...
                    if strand == 1:
                        ...
                        "0\t",
                    print
                    "{}\t{}\t{}\t".format(sRId, resPrint[2] + 1, mapq),
                    print
                    sCigar,
                    print
                    "\t*\t0\t0\t",
                    print
                    sQSeq[resPrint[4] : resPrint[5] + 1] if strand == 0 else sQRcSeq[
                        resPrint[4] : resPrint[5] + 1
                    ],
                    print
                    "\t",
                    if sQQual:
                        if strand == 0:
                            print
                            sQQual[resPrint[4] : resPrint[5] + 1],
                        else:
                            print
                            sQQual[-resPrint[4] - 1 : -resPrint[5] - 1 : -1]
                    else:
                        print
                        "*",

                    print
                    "\tAS:i:{}".format(resPrint[0]),
                    print
                    "\tNM:i:{}\t".format(len(sA) - sA.count("|")),
                    if resPrint[1] > 0:
                        print
                        "ZS:i:{}".format(resPrint[1])
                    else:
                        print
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--bProtien",
        action="store_true",
        help="Do protein sequence alignment. Without this option, the ssw_test will do genome sequence alignment. [default: False]",
    )
    parser.add_argument(
        "-a",
        "--sMatrix",
    ...
    parser.add_argument(
        ...
        "--bPath",
        action="store_true",
        help="Return the alignment path. [default: False]",
    )
    parser.add_argument(
        "-f",
        "--nThr",
    ...
    parser.add_argument(
        ...
        "--bSam",
        action="store_true",
        help="Output in SAM format. [default: no header]",
    )
    parser.add_argument(
        "-header",
        "--bHeader",
        action="store_true",
        help="If -s is used, include header in SAM output.",
    )
