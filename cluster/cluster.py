...
def sequence_clustering(protein_list):
    ...
    protein_sim_mat = np.load('seq_sim_mat.npy').astype(np.float32)
    ...
    for thre in [0.3, 0.4, 0.5, 0.6,0.0001]:
        ...
        print('thre', thre, 'total num of sequences', len(protein_list), 'num of clusters', max(Prot_clusters), 'max length', max(len_list))
...
# split train,valid,test sets for cluster-based cross validation
