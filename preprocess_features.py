...
if __name__ == "__main__":
    ...
    for line in f.readlines()[
        1:
    ]:  # if the file has headers and pay attention to the columns (whether have peptide binding site labels)
    ...
    print
    "num of peptides", len(pep_len), "pad_pep_len", pad_pep_len
    print
    "seq_set", len(seq_len), "pad_prot_len", pad_prot_len
    print
    "num of peptide ss", len(pep_ss_len), "pad_pep_len", pad_pep_len
    print
    "seq_ss_set", len(seq_ss_len), "pad_prot_len", pad_prot_len
    ...
    with open(
        "./dense_feature_dict/Protein_pssm_dict"
    ) as f:  # value: (sequence_length, 20) without sigmoid
        protein_pssm_dict = pickle.load(f)

    with open(
        "./dense_feature_dict/Protein_Intrinsic_dict"
    ) as f:  # value: (sequence_length, 3): long, short, anchor
        protein_intrinsic_dict = pickle.load(f)

    with open(
        "./dense_feature_dict/Peptide_Intrinsic_dict_v3"
    ) as f:  # value: (sequence_length, 3): long, short, anchor
    ...
    for line in f.readlines()[1:]:
        ...
        if pep not in peptide_dense_feature_dict:
            feature = padding_intrinsic_disorder(
                peptide_intrinsic_dict[pep], pad_pep_len
            )
        ...
        if seq not in protein_dense_feature_dict:
            ...
            feature_intrinsic = padding_intrinsic_disorder(
                protein_intrinsic_dict[seq], pad_prot_len
            )
