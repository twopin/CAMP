...
parser.add_option('-l', '--l', default=50,
                  help='The padding length for peptides l')
parser.add_option('-f', '--f', default=0.9,
                  help='The fraction of GPU memory to process the model f'
                  )
parser.add_option('-m', '--m', default=1,
                  help='Using the binary model m=1 or the multi-level model m=2'
                  )
parser.add_option('-p', '--p', default=800,
...
if model_mode == 1:
    ...
    np.save('./example_prediction/bs_test_peptide',
            np.array(pep_sequence))
    np.save('./example_prediction/bs_test_protein',
            np.array(prot_sequence))
else:
    ...
    np.save('./example_prediction/bs_test_peptide',
            np.array(pep_sequence))
    np.save('./example_prediction/pred_pep_bindingsites',
            np.array(pred_bs))
    np.save('./example_prediction/bs_test_protein',
            np.array(prot_sequence))
