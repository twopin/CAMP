...
parser.add_option("-f","--f",default=0.9, help="The fraction of GPU memory to process the model f")
parser.add_option("-m","--m",default=1, help="Using the binary model m=1 or the multi-level model m=2")
parser.add_option("-p","--p",default=800, help="The padding length for proteins p, the length is set to default value when predicting")
...
def load_example(model_mode):
	...
	if model_mode==1:
		return X_pep, X_p, X_SS_pep, X_SS_p, X_2_pep, X_2_p, X_dense_pep, X_dense_p, pep_sequence, prot_sequence
	else:
...
if model_mode==1:
	...
	#model.summary()
	print('Finish loading CAMP.')
	X_pep, X_p, X_SS_pep, X_SS_p, X_2_pep, X_2_p, X_dense_pep, X_dense_p, pep_sequence, prot_sequence, X_pep_mask, X_bs_flag = load_example(model_mode)
