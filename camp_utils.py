...
def split_train_test_clusters(clu_thre, n_fold):
    ...
    return (
        pair_train_clusters,
        pair_test_clusters,
        peprot_train_clusters,
        peprot_test_clusters,
        prot_train_clusters,
        prot_test_clusters,
        Pep_cluster_dict,
        Prot_cluster_dict,
...
def split_data(
    setting, clu_thre, n_fold, X_pep_seq, X_prot_seq
):  # X_pep_seq & X_prot_seq are the distinct sequence ids in the cluster dicts.
    pep_id_list = []
    prot_id_list = []
    ...
    if setting == "new_protein":
        ...
        for fold in range(n_fold):
            ...
            valid_idx = np.random.choice(
                train_idx, int(len(train_idx) / 10), replace=False
            )
            valid_idx_list.append(valid_idx)
            test_idx_list.append(test_idx)
            print
            "fold", fold, "train ", len(train_idx), "test ", len(
                test_idx
            ), "valid ", len(valid_idx)
        ...
        for fold in range(n_fold):
            pep_train, pep_test = (
                peprot_train_clusters[fold],
                peprot_test_clusters[fold],
            )
        ...
        for fold_x in range(int(n_fold ** 0.5)):
            for fold_y in range(int(n_fold ** 0.5)):
                pep_train, prot_train = (
                    peprot_train_clusters[fold_x],
                    prot_train_clusters[fold_y],
                )
                pep_test, prot_test = (
                    peprot_test_clusters[fold_x],
                    prot_test_clusters[fold_y],
                )
                pep_train = set(pep_train)
                prot_train = set(prot_train)
                ...
                for ele in range(n_sample):
                    if (
                        Pep_cluster_dict[pep_id_list[ele]] in pep_train
                        and Prot_cluster_dict[prot_id_list[ele]] in prot_train
                    ):
                        train_idx.append(ele)
                    elif (
                        Pep_cluster_dict[pep_id_list[ele]] in pep_test
                        and Prot_cluster_dict[prot_id_list[ele]] in prot_test
                    ):
                        test_idx.append(ele)
                train_idx_list.append(train_idx)
                valid_idx = np.random.choice(
                ...
                test_idx_list.append(test_idx)
                print
                "fold", fold_x * int(n_fold ** 0.5) + fold_y, "train ", len(
                    train_idx
                ), "test ", len(test_idx), "valid ", len(valid_idx)
