...
def train(train_data, valid_data, test_data, n_epoch=100):
	...
	for e in range(n_epoch):
		...
		pred_test = model.predict([np.array(test_pep),np.array(test_prot),np.array(test_pep_ss),np.array(test_prot_ss),\
									...
									np.array(test_pep_mask),np.array(test_bs_flag)],batch_size=batch_size)[0]
		pred_test_d_bs = model.predict([np.array(test_pep),np.array(test_prot),np.array(test_pep_ss),np.array(test_prot_ss),\
				np.array(test_pep_phy),np.array(test_prot_phy),np.array(test_pep_dense),np.array(test_prot_dense),\
				np.array(test_pep_mask),np.array(test_bs_flag)],batch_size=batch_size)[1]
...
def load(input_file):
	...
	return X_pep, X_prot, X_pep_ss, X_SS_p, X_pep_phy, X_prot_phy, X_pep_dense, X_prot_dense, X_pssm, Y, peptide_sequence, protein_sequence, X_pep_mask, Y_pep_bs, X_bs_flag
...
if __name__ == '__main__':
    ...
    X_pep, X_prot, X_pep_ss, X_SS_p, X_pep_phy, X_prot_phy, X_pep_dense, X_prot_dense, X_pssm, Y, peptide_sequence, protein_sequence, X_pep_mask, Y_pep_bs, X_bs_flag = load(input_file)
